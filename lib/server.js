...
server.unifiedServer = (req, res) => {
  ...
  req.on('data', (data) => {
    buffer += decoder.write(data); //чтобы декодированная строка не содержала неполных многобайтовых символов
  });
  
  req.on('end', () => {
    buffer += decoder.end(); //данные прочитаны досылаются послединие куски для сбора данных в буфере
    
    let chosenHandler = (server.router[trimmedPath]) ? server.router[trimmedPath] : handlers.notFound; //path or 404
  
    // If the request is within the public directory use to the public handler instead
    chosenHandler = trimmedPath.includes('public') ? handlers.public : chosenHandler; //<-- если запрос находится в общем каталоге используем public повторно чтобы вызвать все файлы из папки
    
    const data = { //данные для оптравки на обработку сервера сгрупированы для удобства
...
server.router = {
  "" : handlers.index,
  "home": handlers.index,
  'favicon.ico' : handlers.favicon, //<-- маршрут для фавикона
  'public' : handlers.public, //<-- для наших статичных публичных данных(стили, картинки) public/
